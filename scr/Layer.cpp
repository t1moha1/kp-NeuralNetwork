...
Layer::Layer(const Eigen::MatrixXd& weights, const Eigen::VectorXd& biases,
             Activation activation)
    : weights(weights), biases(biases), activation(std::move(activation)) {}

Layer::Layer(int input_size, int output_size, Activation activation)
  ...
  for (int i = 0; i < output_size; ++i)
    for (int j = 0; j < input_size; ++j) weights(i, j) = dist(gen);

  biases = Eigen::VectorXd(output_size);
  for (int i = 0; i < output_size; ++i) biases(i) = dist(gen);
}
