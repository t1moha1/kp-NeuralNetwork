...
AdamOptimizer::AdamOptimizer(int num_layers, double learning_rate, double beta1,
                             double beta2, double epsilon)
    : lr(learning_rate), b1(beta1), b2(beta2), eps(epsilon), t(0) {
...
void AdamOptimizer::update(int layer_index, Eigen::MatrixXd& W,
                           Eigen::VectorXd& b, const Eigen::MatrixXd& dW,
  ...
  v_w[layer_index] =
      b2 * v_w[layer_index] + (1 - b2) * dW.array().square().matrix();
  m_b[layer_index] = b1 * m_b[layer_index] + (1 - b1) * db;
  v_b[layer_index] =
      b2 * v_b[layer_index] + (1 - b2) * db.array().square().matrix();
